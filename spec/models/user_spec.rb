...
RSpec.describe User, type: :model do
  ...
  describe "Validations" do
    ...
    it "requires a password" do
      # Given - a new user with all valid attribs except for a missing password
      user = User.new valid_attributes({password: nil})
      # When - trying to save the user record
      user.save
      # Then - get attached errors that include a :password key
    ...
    it "requires an email with a valid format" do
      user = User.new valid_attributes({email: "bob@"})
      expect(user).to be_invalid
    end
  end
  ...
  describe "generating password hash" do
    it "generate a password digest" do
